...
class BundleBuildAction : AnAction(
    "Bundle Build",
    "Run gradle :bundle_runtime:assembleDebug",
    IconLoader.getIcon("/icons/debugBuild.svg", BundleBuildAction::class.java)
) {
    override fun actionPerformed(e: AnActionEvent) {
        val project = e.project ?: return
        runBundleBuild(project)
    }
    ...
    private fun runBundleBuild(project: Project) {
        val basePath = project.basePath ?: return
        val buildId = Object()
        val title = "Bundle Build"
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                buildViewManager.onEvent(buildId, OutputBuildEventImpl(buildId, "$ ./gradlew :bundle_runtime:assembleDebug\n", true))
                buildViewManager.onEvent(buildId, OutputBuildEventImpl(buildId, "Working directory: $basePath\n\n", true))
                buildViewManager.onEvent(
                    buildId,
                    MessageEventImpl(buildId, MessageEvent.Kind.INFO, null, "Running ./gradlew :bundle_runtime:assembleDebug...", null)
                )

                val process = ProcessBuilder("./gradlew", ":bundle_runtime:assembleDebug")
