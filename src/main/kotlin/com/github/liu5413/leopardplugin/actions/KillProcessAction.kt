...
class KillProcessAction : AnAction(
    "Kill Process",
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            ...
            ApplicationManager.getApplication().invokeLater {
                when {
                    devices.isEmpty() -> AdbHelper.showNoBuild(project, "Kill Process", "No connected devices found")
                    devices.size == 1 -> runKillProcess(project, devices[0])
                    else -> AdbHelper.showDeviceChooser(project, devices) { device -> runKillProcess(project, device) }
                }
            }
        }
    }
    ...
    private fun runKillProcess(project: Project, device: AdbHelper.DeviceInfo) {
        val basePath = project.basePath ?: return
        val buildId = Object()
        val title = "Kill Process"
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                val cmd = "am force-stop ${AppConstants.PACKAGE_NAME}"
                buildViewManager.onEvent(buildId, OutputBuildEventImpl(buildId, "$ adb -s ${device.serial} shell $cmd\n", true))
                ...
                val result = AdbHelper.executeShellCommand(project, device.serial, "am", "force-stop", AppConstants.PACKAGE_NAME)
