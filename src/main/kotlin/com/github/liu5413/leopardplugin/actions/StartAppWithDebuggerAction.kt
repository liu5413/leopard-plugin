...
class StartAppWithDebuggerAction : AnAction(
    ...
    private fun showDeviceChooser(project: Project, devices: List<DeviceInfo>) {
        JBPopupFactory.getInstance()
            ...
            .setItemChosenCallback { device -> runStartAppWithDebugger(project, device) }
            .createPopup()
    ...
    private fun showNoBuild(project: Project, message: String) {
        ...
        val buildDescriptor = DefaultBuildDescriptor(
            buildId, "Start App with Debugger", project.basePath ?: "", System.currentTimeMillis()
        )
        val buildViewManager = project.getService(BuildViewManager::class.java)
        buildViewManager.onEvent(buildId, StartBuildEventImpl(buildDescriptor, "Start App with Debugger"))
        BuildContentManager.getInstance(project).getOrCreateToolWindow().show()
    ...
    private fun runStartAppWithDebugger(project: Project, device: DeviceInfo) {
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                val adb = AdbHelper.resolveAdbPath(project)
                val cmd = "$adb -s ${device.serial} shell am start -D -n $MAIN_ACTIVITY"
                ...
                buildViewManager.onEvent(
                    buildId,
                    MessageEventImpl(buildId, MessageEvent.Kind.INFO, null, "Starting app $PACKAGE_NAME with debugger on ${device.model}...", null)
                )

                val process = ProcessBuilder(adb, "-s", device.serial, "shell", "am", "start", "-D", "-n", MAIN_ACTIVITY)
